...
export async function exportGLB(
  ...
  updateMedia(media.attachments, initial, 0, 0);
  ...
  if (animated) {
    ...
    const samples = tracked.map((node) => ({
      ...
      times: number[] = [];
    for (let i = 0; i <= 180; i++) {
      const pose = evaluateSpread(compiled, i, drivers);
      ...
      updateMedia(media.attachments, pose, 0, i / 30);
      times.push(i / 30);
    ...
    const start = evaluateSpread(compiled, 0, drivers);
    for (const part of [...start.parts, ...assemblyTabParts(compiled, start)])
      setMatrix(parts.get(part.id)!, part.matrix);
    updateMedia(media.attachments, start, 0, 0);
  }
  try {
