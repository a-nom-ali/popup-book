...
export function createPaperMesh(part: PaperPart): THREE.Group {
  ...
  geometry.setAttribute(
    'uv',
    new THREE.Float32BufferAttribute(
      points.flatMap((p) => [p[0] / 100, p[1] / 100]),
      2,
    ),
  );
  ...
  group.name = `part_${part.id.replaceAll(/[^a-zA-Z0-9_]/g, '_')}`;
  group.userData.partId = part.id;
  group.add(mesh);
  for (const polygon of [part.polygon, ...part.holes]) {
    const outline = new THREE.BufferGeometry().setFromPoints(
      [...polygon, polygon[0]].map((p) => new THREE.Vector3(p[0], p[1], 0.035)),
    );
    const line = new THREE.Line(
      outline,
      new THREE.LineBasicMaterial({
        color: part.role === 'page' ? '#d0cabb' : '#3e493e',
        transparent: true,
        opacity: 0.45,
      }),
    );
