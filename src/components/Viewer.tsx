...
export default function Viewer({ compiled }: { compiled: CompiledSpread }) {
  ...
  useEffect(() => {
    ...
    reset.current = () => { camera.position.set(275, -350, 300); controls.target.set(0, 0, 28); controls.update(); }; reset.current();
    ...
    let previous: CompiledSpread | undefined, previousAngle = -1, previousDrivers = '', frame = 0, generation = 0, active = true, lastTime = performance.now();
    ...
    const pick = (e: PointerEvent) => {
      ...
      if (digitalId) { const a = media.find(a => a.id === digitalId); if (a) { a.clicked = true; a.elapsed = 0; a.action?.reset().play(); } }
      useStudio.getState().set({ selectedId: digitalId ?? hit?.object.userData.partId ?? null });
    };
    renderer.domElement.addEventListener('pointerdown', pointerDown); renderer.domElement.addEventListener('pointerup', pick);
    const animate = () => {
      frame = requestAnimationFrame(animate); const s = useStudio.getState(), data = latest.current, drivers = JSON.stringify(s.drivers), now = performance.now(), delta = Math.min((now - lastTime) / 1000, .1); lastTime = now;
      ...
      if (data !== previous || s.angle !== previousAngle || drivers !== previousDrivers) {
        if (data !== previous) {
          ...
          const previousMeshes = new Map(meshes); book.clear(); meshes.clear();
          for (const [id, mesh] of previousMeshes) { const stillAuthored = data.spread.mechanisms.some(m => id.startsWith(`${m.id}:`)); const solved = pose.parts.some(p => p.id === id); if (stillAuthored && !solved && previous?.spread.id === data.spread.id) { meshes.set(id, mesh); book.add(mesh); } else disposeObject(mesh); }
          media.forEach(a => disposeObject(a.group)); media = []; const current = ++generation;
          loadMedia(data, pose).then(result => { if (!active || current !== generation) { result.attachments.forEach(a => disposeObject(a.group)); return; } media = result.attachments; media.forEach(a => book.add(a.group)); if (result.errors.length) useStudio.getState().set({ notice: result.errors.join(' ') }); });
        }
    ...
    return () => { active = false; cancelAnimationFrame(frame); observer.disconnect(); controls.dispose(); renderer.domElement.removeEventListener('pointerdown', pointerDown); renderer.domElement.removeEventListener('pointerup', pick); disposeObject(scene); renderer.dispose(); renderer.domElement.remove(); };
  }, []);
